...
### Acceptance checklist
- A `MERGE`-mode re-import of a portfolio with manually-entered cost basis no longer clears that value.
- Any code path that lets a user attach an ISIN to a shared `Security` is authorized or scoped to that user, not global shared-data mutation by any authenticated user.
- `PortfolioImportServiceTest` and `PortfolioImportControllerTest` (or equivalent) exist and cover merge/replace branching, confirmation gating, ownership, and duplicate consolidation.
- A CSV import preview at `maxRows()` no longer issues one query per row for ISIN resolution; query count is bounded regardless of row count.
- All FIH fixes and tests pass alongside the existing FI1/FI2 test suite with no regressions.
...
## Group TA — AI Investment Thesis (Vertex AI Gemini)

Goal: give the platform an AI-generated investment thesis (bull case, bear case, risks, invalidation conditions) that interprets VIS-computed financial context without ever replacing VIS's deterministic engines, using **Google Cloud Vertex AI's managed Gemini API** as the inference engine instead of a locally fine-tuned model. This group formally supersedes the local `google/gemma-3-27b-it` teacher pipeline and the `google/gemma-3-4b-it` QLoRA adapter path started in `vis-model-training/`, which was closed `NO_GO` on 2026-08-24 after failing its strict-output capability gate three times in a row (see `vis-model-training/reports/teacher/train-05-failure-and-qlora-pause.md`). The failure mode was structural: the self-hosted checkpoint could not reliably produce schema-conforming JSON (it wrapped output in Markdown fences even under corrected prompting) and scored far below threshold on grounding, risk coverage, and decision-support safety. A managed model with native controlled-generation (JSON-schema-constrained decoding) removes that specific failure mode without requiring any GPU training infrastructure, adapter, or fine-tuning run.

Independent of Group K: Vertex AI is a managed Google Cloud API reachable from local/dev environments via a service account, the same way FMP is reachable via an API key before any Cloud Run deployment exists. TA does not depend on K1–K3 completing first, and K1–K3 do not depend on TA.

**What carries over unchanged from `vis-model-training/`:** the task contract (`schemas/thesis-input.schema.json`, `schemas/thesis-output.schema.json`), the system prompt baseline (`prompts/system-prompt-v2.txt`), the dataset/response validator CLI (TRAIN-02), the 50-case benchmark harness and dataset (TRAIN-03), the 500-scenario synthetic generator and dataset (TRAIN-04), and the runtime-integration contract design (TRAIN-12 — request/response envelope, error codes, deterministic fallback, versioning, responsibility matrix, audit policy).

**What this group retires:** the teacher/critic generation pipeline (TRAIN-05), dataset curation for fine-tuning (TRAIN-06), the QLoRA environment and pilot training (TRAIN-07/TRAIN-08), adapter comparative evaluation and data/training iteration (TRAIN-09/TRAIN-10), and adapter packaging/release (TRAIN-11) — none of these apply to a managed-API engine with no adapter artifact. The local Gemma/QLoRA path itself stays paused under its own documented re-entry criteria; this decision does not reopen or judge it further.

### Phase TA1: Governance, Model Selection & Reuse Assessment *(complete)*
- Record an ADR (`vis-model-training/docs/adr/ADR-002-vertex-gemini-selection.md`) documenting the switch from the local Gemma/QLoRA path to Vertex AI Gemini as the production inference engine for the Investment Thesis Agent, with an explicit reuse inventory (carried-over vs. retired items, as summarized above) and an explicit statement that no further local fine-tuning spend is authorized under this decision.
- New governance review distinct from TRAIN-00's open-weight review: confirm Vertex AI Gemini's terms of service for sending VIS-derived financial context (not raw user PII) to a managed third-party API, confirm the API is not used to train Google's models on customer data by default, and record the region/data-residency choice for `VERTEX_AI_LOCATION`. Update `vis-model-training/docs/governance/data-and-model-licenses.md` and `docs/governance/data-policy.md` accordingly.
- Confirm the decision-support and secrets principles from `specs/mission.md` (conservative defaults, no credential in source control, immutable historical data) extend cleanly to a managed-API integration; no exception is being requested.
- Establish initial cost-governance policy: a budget estimate for the TA3 benchmark run (roughly 550 scored calls plus the real-ticker set added in TA3), and an explicit decision on production-time rate limiting and/or role gating for on-demand generation (Phase TA4/TA5), mirroring the existing seed-pack cost/quota governance pattern (`specs/roadmap.md` → H8).
- Deliverable: `vis-model-training/docs/adr/ADR-002-vertex-gemini-selection.md`, updated governance docs, reuse-inventory table, cost-governance policy note.

### Phase TA2: Vertex AI Prompt Contract & Structured-Output Adaptation *(complete)*
- Reuse `thesis-input.schema.json` / `thesis-output.schema.json` unchanged as the contract; adapt the Gemini request to use controlled generation (`responseMimeType: application/json` plus a `responseSchema` derived from `thesis-output.schema.json`) so structural JSON conformance is enforced by the API itself — directly targeting the Markdown-fence failure mode that closed TRAIN-05.
- No grounding tools are enabled (no Search grounding, no Vertex AI Search, no function calling to external data sources): the model reasons only over the VIS-supplied financial-context JSON, consistent with mission.md's "Data before opinion" principle and the existing rule against using knowledge outside the supplied input.
- Initial model candidate: a current-generation Gemini Flash-tier model (cost/latency-appropriate for a per-request interpretation task), with a Pro-tier model as the fallback candidate if Flash-tier fails the TA3 capability gate on grounding, risk coverage, or decision-support safety. The final choice is decided by the TA3 gate result, not assumed here.
- Decoding configuration: `temperature: 0` (or the lowest value the API accepts as its deterministic/greedy-equivalent setting), fixed `maxOutputTokens` aligned with the benchmark's observed output length. Document explicitly that Vertex AI does not guarantee bit-exact determinism across identical calls the way local greedy decoding did in TRAIN-03; TA3's benchmark design must tolerate and report this (e.g., flag any case whose classification or schema validity differs across repeated calls, rather than assuming single-call results are fully reproducible).
- Few-shot policy: the 10 manual examples from TRAIN-01 remain reference/documentation examples and are **not** injected into the Gemini prompt as few-shot content in the first iteration, keeping parity with the zero-shot design TRAIN-03 already validated. Adding few-shot examples is an explicit lever to revisit only if TA3 fails the gate on a category few-shot could plausibly fix — not a default.
- Reuse TRAIN-02's validator CLI unchanged as the schema/semantic gate applied to every Gemini response before it can reach a user: evidence-field grounding, `INSUFFICIENT_DATA` handling, `humanReviewRequired` logic, and the ban on `BUY`/`SELL`/`HOLD` language.
- Deliverable: `vis-model-training/config/vertex-gemini-v1.json` (model id, location, generation config, response-schema binding); a new prompt variant only if controlled generation requires wording changes beyond `system-prompt-v2.txt`.

### Phase TA3: Capability Benchmark on Vertex AI Gemini (replaces TRAIN-05) *(complete)*
- **Result: GO.** Live run against `gemini-2.5-flash` (`europe-west1`), 574/574 cases (50 TRAIN-03 + 500 TRAIN-04-converted + 24 real-ticker), 100% JSON validity/schema compliance vs. Gemma's 0%, zero genuine semantic-validator violations, zero knowledge leakage across 20 scored real-ticker cases, 68-case human review accept rate 0.8088 (clears the 0.80 gate floor). Full detail: `vis-model-training/specs/2026-08-27-ta3-vertex-capability-benchmark/validation.md` and `vis-model-training/reports/vertex/gemini-2.5-flash-v1/`. One systematic gap found and left for TA4 (see below), not blocking this GO.
- Framing: Gemini is not a "teacher" generating training data for a smaller student model, as Gemma was in TRAIN-05 — it **is** the production inference engine. The TRAIN-05 capability-probe gate is therefore repurposed from "is this output good enough to seed a training dataset" to "is this output good enough to show a user directly." The bar is reapplied unchanged; only its interpretation changes.
- Rerun the existing TRAIN-03 baseline benchmark (`datasets/benchmark/base-benchmark-v1.jsonl`, 50 cases) and the TRAIN-04 scenario catalog (`datasets/candidates/scenarios-v1.jsonl`, 500 scenarios) against Vertex AI Gemini, using the same automatic metrics (JSON validity, schema compliance, classification accuracy, evidence-field precision, unsupported-numeric-claim rate, prohibited-recommendation rate, human-review accuracy) and the same human-review rubric (`rubrics/manual-review-v1.json`) used for the Gemma baseline, so results are directly comparable.
- **New real-ticker knowledge-leakage test set:** TRAIN-04's 500 scenarios all use synthetic `VIS*`-prefixed symbols specifically to prevent the evaluated model from drawing on pretrained knowledge — this tests nothing about whether a large general-purpose model overrides supplied evidence with what it actually knows about a real company, which is a materially bigger risk for Gemini than it was for the smaller, domain-scoped Gemma checkpoint. Build a supplementary set of 20–30 cases (`datasets/benchmark/real-ticker-knowledge-leakage-v1.jsonl`) using real, well-known tickers already referenced elsewhere in this roadmap's demo/seed lists (e.g. `AAPL`, `MSFT`, `KO`, `JNJ`), each with deliberately altered or contradictory supplied values (wrong sector, fabricated negative FCF trend, understated price, etc.). Extend the manual-review rubric with an explicit `KNOWLEDGE_LEAKAGE` criterion — did the thesis state anything about the real company that is true in reality but absent from the supplied input, or did it correctly follow the (deliberately wrong) supplied evidence — since this failure mode is not mechanically detectable by the schema/semantic validator alone.
- Reapply the same strict capability gate that closed TRAIN-05 (`config/capability-probe-gate.json`) as the go/no-go threshold for adopting Gemini, extended to require a passing `KNOWLEDGE_LEAKAGE` rate on the new real-ticker set — this is a substitution of engine under the same bar (plus one bar Gemma's synthetic-only evaluation never had to clear), not a relaxation of it.
- Produce a written comparison against the closed Gemma baseline (`reports/baseline/gemma-3-4b-it-v1/`, `reports/teacher/train-05-*`) so the switch is evidence-based rather than assumed, and record the actual Vertex AI spend against the TA1 budget estimate.
- Gate: TA4 production integration does not start unless Gemini clears the same capability gate Gemma failed on grounding, risk coverage, schema conformance, and decision-support safety, **and** the new real-ticker knowledge-leakage check.
- Deliverable: `vis-model-training/datasets/benchmark/real-ticker-knowledge-leakage-v1.jsonl`, `vis-model-training/reports/vertex/gemini-2.5-flash-v1/`.

### Phase TA4: Runtime Integration Contract & VIS Backend Client *(complete)*
- **Result:** `it.mazzoni.vis.thesis` (client, persistence, 3 endpoints + ADMIN review queue, Redis rate limiting, deterministic fallback) implemented, 495/495 backend tests + 3/3 Docker-verified `ThesisRateLimiterIT` pass. `system-prompt-v3.txt` fix adopted and re-verified at full scale (63/63, 100%, up from 0/68 on v2) after an initial mis-run was caught and corrected. `THESIS_AGENT_ENABLED` stays `false` — flipping it to `true` is a separate, later operational decision. Full detail: `vis-model-training/specs/2026-08-28-ta4-runtime-integration-contract/validation.md`, `vis-model-training/reports/vertex/gemini-2.5-flash-v1-promptv3/`.
- **Carries over from TA3's GO decision: adopt the verified `humanReviewRequired` prompt fix.** TA3 found that `prompts/system-prompt-v2.txt` Rule 9 never fires `humanReviewRequired=true` for a `STRONGLY_DECLINING` trend alongside a positive margin of safety (a value-trap pattern) — a live-tested one-clause fix reached 100% reliability where its condition held, zero false positives on controls (`vis-model-training/reports/vertex/gemini-2.5-flash-v1/experiments/human-review-rule-experiment.md`). TA4 must apply this fix to the production prompt, re-run the full 574-case corpus, and re-verify before treating it as closed — not assumed from the 19-case experiment alone.
- Adapt TRAIN-12's runtime contract (request/response envelope, error codes, versioning/deprecation policy, responsibility matrix, `humanReviewRequired`/`dataWarnings` audit policy) by repointing model-identity fields at the Vertex AI Gemini model id/version instead of an adapter, and replacing `adapterChecksum` with the pinned Gemini model version/endpoint identifier — no adapter artifact exists in this path.
- **Model version pinning:** `GEMINI_MODEL_ID` must reference a specific stable, versioned model string, never a floating/auto-updating alias — a self-hosted adapter (TRAIN-12's original assumption) never changes underneath VIS, but a mutable Vertex AI alias can change server-side outside VIS's control, silently invalidating the TA3 benchmark's conclusions. Document Google's deprecation/EOL notice window for the pinned version and add an operational reminder to re-run the TA3 gate before migrating to a newer pinned version.
- **Persistence:** add an `investment_thesis_result` entity/table (Flyway migration) storing, per generated thesis: security/symbol reference, pinned `modelId`/`modelVersion`, `promptVersion`, the exact input snapshot sent to Gemini (for audit/reproducibility), the full parsed output fields (classification, confidence, summary, bull/bear case, risks, assumptions, invalidation conditions, data warnings), `status` (`READY`/`FAILED`/`HUMAN_REVIEW_PENDING`), `requestId`, `latencyMs`, `generatedAt`. Theses are platform-wide reference data (like `ValuationResult`/`ValueScore`), not user-owned. Generation is **on-demand, never automatic on page load** — a thesis is reused until explicitly regenerated, both to control API spend and to keep TRAIN-12's audit trail meaningful. Mark a stored thesis `stale` (without auto-regenerating) when the underlying `ValuationResult`/`ValueScore` it was generated from has been refreshed since `generatedAt`.
- **Endpoints:** `POST /api/v1/securities/{symbol}/thesis/generate` (any authenticated role, rate-limited — see below) starts an async generation and returns a `thesisRunId`; `GET /api/v1/securities/{symbol}/thesis/runs/{thesisRunId}/status` polls run status, mirroring JC2's job-run status polling; `GET /api/v1/securities/{symbol}/thesis` returns the latest persisted thesis (or `NOT_GENERATED`) without triggering generation. Generation runs asynchronously and never blocks the synchronous H4C review-page aggregation — LLM latency (seconds) does not fit that endpoint's existing fast-path contract.
- **Cost/rate governance:** add a configurable `THESIS_GENERATION_DAILY_LIMIT` (per-user, conservative default) enforced on the generate endpoint; return a clear rate-limit error (not a generic 500) when exceeded. Bulk/batch generation during seeding stays an explicit, separate, ADMIN-only action if it is added at all — it is not implied by this phase.
- **Human review queue:** persist any `humanReviewRequired=true` or non-empty-`dataWarnings` thesis with `status=HUMAN_REVIEW_PENDING` in the same table, and expose `GET /api/v1/admin/thesis/review-queue` (ADMIN only) listing pending items, mirroring the existing alert-queue pattern (G2) rather than introducing a new subsystem.
- **Test isolation:** default backend tests mock `InvestmentThesisClient` and never call live Vertex AI. Add `backend/src/test/resources/application-vertexkey.yml` (gitignored, mirroring `application-fmpkey.yml`) and `@ActiveProfiles({"test","vertexkey"})` for the small set of integration tests that do call the real API; these are excluded from the default CI run.
- Add a VIS backend `InvestmentThesisClient` (Spring Boot, in the main repository, outside `vis-model-training/` — matching TRAIN-12's own scope boundary) that calls Vertex AI's Gemini API via Google Cloud's Java client library, authenticated by a service account through Secret Manager (never a static API key in source, consistent with mission.md's secrets principle). Add a `.gitignore` rule for the local service-account JSON key file (`GOOGLE_APPLICATION_CREDENTIALS`) before it is ever created locally, matching the existing `.env` / `**/application-fmpkey.yml` pattern.
- Preserve TRAIN-12's deterministic-fallback rule: on error, timeout, or non-conforming output after configured retries, persist `status=FAILED` with `classification: UNDER_REVIEW`, `humanReviewRequired: true`, no bull/bear case, and a tracked error reason — never a silent second-model fallback and never partial/malformed text shown to a user.
- Reuse TRAIN-12's acceptance suite unchanged: full TRAIN-03 benchmark categories plus TRAIN-04 scenarios (plus the new real-ticker set) rerun against the live integration; contract tests for malformed input (`INPUT_SCHEMA_INVALID`), simulated timeout (deterministic fallback), and a non-`APPROVED` model version being rejected at configuration level.
- Deliverable: `vis-model-training/docs/integration/*` (adapted from TRAIN-12), the VIS-side `InvestmentThesisClient`, the `investment_thesis_result` migration/entity/repository, the three endpoints above, and their tests.

### Phase TA5: AI Investment Thesis Frontend *(complete)*
- Add an "AI Investment Thesis" panel to the Security Review page (H4A/H4C), lazy-loaded and **on-demand** — a "Generate AI Thesis" call-to-action, not automatic content on page load, consistent with TA4's async design and cost governance.
- States: not-yet-generated (call-to-action), generating (progress indicator, polls the `thesisRunId` status endpoint), ready (classification badge, summary, bull case / bear case lists with each claim's `evidenceFields` shown as inline references back to the relevant review-page section, key risks, key assumptions, invalidation conditions, data warnings), **human-review-pending** (a visually distinct banner — "This thesis is flagged for human review" — that never renders bull/bear case as a finished, ready-to-act-on recommendation), **stale** (underlying valuation/score has been refreshed since generation; offer a regenerate action), failed (error detail + retry), and rate-limited (clear message when `THESIS_GENERATION_DAILY_LIMIT` is hit, not a generic error).
- Show model/prompt provenance (model version, prompt version, `generatedAt`, `requestId`) in a collapsible raw-JSON inspector, consistent with the demo pages' existing pattern.
- Display the MiFID II decision-support disclaimer adjacent to the thesis, exactly as on every other Fair Value/Value Score output.
- Add an ADMIN-only review queue view consuming `GET /api/v1/admin/thesis/review-queue`, listing all `HUMAN_REVIEW_PENDING` theses across symbols so flagged output is actually triaged rather than silently stored.
- Acceptance checklist:
  - Generation is on-demand and never fires automatically on page load.
  - A human-review-pending thesis is never presented as a finished recommendation; the banner is unmissable.
  - A stale thesis is visibly flagged rather than silently shown as current.
  - The MiFID II disclaimer is present whenever thesis content is shown.
  - The ADMIN review queue lists every pending item and links back to its symbol/review page.

### Phase TA6: Trend-Direction Correctness Fix *(complete)*
- Fixed `ThesisInputBuilder.classifyTrend` (used for `revenueTrend`/`earningsTrend`/`freeCashFlowTrend`, every AI thesis, every sector, since TA4 shipped 2026-08-28): it assumed the *opposite* array direction from every other reader of `FundamentalSnapshot`'s `*History` lists (`FmpAdapter`, `DemoAnalysisService`, and RM4's own `deriveNetDebtToEbitda` fix all independently confirm newest-first, index 0 = most recent) — flagged as a known risk in `specs/2026-09-02-rm4-ai-thesis-propagation/validation.md` and deliberately left unfixed there pending its own dedicated investigation, per that phase's own recommendation.
- Fix: reverse `history` into chronological order once at the top of the method, then reuse the existing period-change/threshold logic unchanged — same thresholds, same volatility rule, only the direction correction.
- Existing `classifyTrend_*` unit test fixtures rewritten to genuinely newest-first order (the old fixtures happened to pass under either direction and silently documented the wrong convention); one new test constructed so the two directions disagree on the resulting `Trend` value, failing against the pre-fix implementation.
- **Live verification against real FMP data (2026-09-03):** Docker was not available this session, so — rather than the full seed+live-thesis-generation flow RM2/RM3/RM4 used — verification fetched real `/income-statement` revenue history directly from FMP for several real tickers, found INTC's 7-year window has a genuine direction disagreement (true latest period 2024→2025 = −0.47%, `STABLE`; the pre-fix bug reads −7.58%, `DECLINING`), and confirmed via a temporary spike test calling the real `classifyTrend` method with INTC's real fetched values: the pre-fix code returns `DECLINING`, the fix returns `STABLE`, both confirmed by actually running the code (not by hand-computation alone) before the spike was deleted. Full detail: `specs/2026-09-03-ta6-thesis-trend-direction-fix/validation.md`.
- No prompt, schema, `evidenceFields`, or frontend change — the three `Trend` fields' shape and citability were already correct (TA4); only the classification logic underneath is corrected. No backfill of historical `InvestmentThesisResult` rows, matching RM4's own posture for `netDebtToEbitda`.

### Acceptance checklist
- ADR and governance updates record the switch from local Gemma/QLoRA to Vertex AI Gemini, with an explicit reuse/retire inventory against `vis-model-training/`, and a cost-governance policy exists before TA3 spends against the live API.
- Gemini responses are structurally constrained via `responseSchema` and still pass the unchanged TRAIN-02 validator.
- Vertex AI Gemini clears the same capability gate that closed TRAIN-05, including the new real-ticker knowledge-leakage check, evidenced by a benchmark rerun on the same 50-case and 500-scenario datasets plus the new real-ticker set.
- The pinned `GEMINI_MODEL_ID` is a specific stable version, never a floating alias.
- Theses are persisted, generated on-demand (never automatically), and flagged `stale` rather than silently reused past their inputs' freshness.
- The default backend test suite never calls live Vertex AI; live-API tests are gated behind an explicit `vertexkey` profile.
- The VIS backend client enforces the deterministic fallback rule and never displays malformed or unvalidated model output.
- A `HUMAN_REVIEW_PENDING` thesis is never presented to a user as a finished recommendation, and is visible in the ADMIN review queue.
- The AI-generated thesis is visibly separate from, and never a substitute for, the deterministic DCF/Graham/DDM/MoS/Value Score outputs, and carries the MiFID II disclaimer.
- No new local fine-tuning, adapter training, or GPU spend is introduced by this group.
...
### Phase RM1: Data Ingestion Depth (D&A, EBITDA Inputs) *(complete)*
- Confirmed live against FMP Premium for `O`, `PLD`, `SPG` (`/income-statement`, `/balance-sheet-statement`, `/cash-flow-statement`, `/key-metrics`, `/ratios`, `period=annual`): `depreciationAndAmortization` and a precise, FMP-computed `ebitda` are both present directly on `/income-statement` — no derivation needed. Gains/losses on real-estate sales and NOI are confirmed **absent** from every endpoint checked (only generic `otherNonCashItems`/`otherInvestingActivities` catch-alls exist; no property-level expense or NOI line anywhere).
- Added `depreciationAndAmortization`/`ebitda` to `FmpIncomeStatementEntry`, propagated as new history lists (`depreciationAndAmortizationHistory`/`ebitdaHistory`) through `FmpAdapter.toFundamentalSnapshot`, the domain `FundamentalSnapshot` record (Yahoo fallback path defaults to empty lists, per Design Principle 5 — `YahooFinanceAdapter`'s call site needed no changes since it already used the shorter telescoping constructor overload), and the persisted `fundamental_snapshot` table (`V28__fundamental_snapshot_da_ebitda.sql`), wired through `BulkFundamentalsSyncJob`.
- NOI availability resolved: Fallback A (`Revenue − operatingExpenses`) and Fallback B (`operatingIncome + D&A`) were both evaluated and rejected — the only expense breakdown FMP exposes is company-wide, not property-level, so either proxy would silently misrepresent Implied Cap Rate/NAV rather than approximate them. **Fallback C is in effect:** Implied Cap Rate (§4.7) and NAV per share (§4.8) are out of scope for RM1/RM2, `INSUFFICIENT_DATA` per Design Principle 12, gated on a future data-source decision. FFO ships as `Net Income + D&A` only (the gains/losses add-back is also out of scope, same reason), documented as an approximation of the full NAREIT formula.
- FFO/AFFO/Debt-EBITDA/growth/payout (§4.1–§4.6) are fully unblocked for RM2 with the newly available D&A/EBITDA fields.
- Tests: `FmpAdapterTest` (new) verifies the D&A/EBITDA history mapping and the empty-history fallback; full backend suite (509 tests) green after the `FundamentalSnapshot` record/entity change.

### Phase RM2: SectorMetricProfile & REIT Metric Computation *(complete)*
- **Result:** `SectorClassifier.isReit` (REIT/real-estate only, narrower than `isReitOrUtility`) gates a new `SectorMetricService` that computes and persists FFO/AFFO per share, P/FFO, P/AFFO, Net Debt/EBITDA, EBITDA interest coverage, and AFFO payout ratio onto `RatioSnapshot` (Flyway `V29`) for REIT-classified securities, additive to every other sector. `ValueScoreService`'s five pillars branch on `SectorClassifier.isReit`: MoS from a live-computed P/FFO percentile band (new `ValuationHistoryService` `"P_FFO"` metric, reusing Group MA's band machinery) rather than DCF-derived composite fair value; Quality from FFO margin; Safety from Net Debt/EBITDA + EBITDA interest coverage; Growth from FFO-per-share YoY; Dividend from AFFO payout ratio — all first-pass, config-driven thresholds (`SectorMetricProperties`). Every non-REIT sector's formulas are byte-for-byte unchanged (534/534 backend tests pass, zero regressions).
- Implied Cap Rate and NAV per share remain out of scope, per RM1's confirmed NOI-unavailability finding (Fallback C) — not attempted.
- Closed a pre-existing gap found while implementing: RM1's D&A/EBITDA fields were wired only into the nightly `BulkFundamentalsSyncJob`, never into `SeedTickerService`'s seed path — fixed so a freshly-seeded REIT has FFO/AFFO inputs immediately, not only after the next nightly job run. Also fixed a real ordering bug: `SectorMetricService.compute()` must run before `ValueScoreService.compute()` within `SeedTickerService.seedOne()`, or a REIT's first score is silently computed against null sector metrics.
...
- **Bug found and fixed via this phase's own live smoke test:** `deriveNetDebtToEbitda` read the *oldest* year in a multi-year history as "latest" (inherited, pre-existing indexing bug from the pre-RM4 `operatingIncomeHistory`-based version) — produced `O`'s netDebtToEbitda as 35.4x instead of the correct ~9.1x. Root-caused against real `fundamental_snapshot` columns and fixed (index 0, not the last index, is FMP's "most recent" convention). **A related, larger-scope bug was found but deliberately left unfixed and flagged for a follow-up phase:** `classifyTrend`'s trend-direction logic plausibly reads `revenueTrend`/`earningsTrend`/`freeCashFlowTrend` in reverse-chronological order for every AI thesis generated since TA4 (2026-08-28) — fixed separately in Group TA's own Phase TA6.

### Phase RM5: REIT-Aware Composite Fair Value & Margin of Safety *(complete)*
- **Found by direct user observation of a live AI thesis on staging, root-caused against real code:** `ValuationService.calculate()` had zero `SectorClassifier.isReit` branching — `compositeFairValue`/`marginOfSafety` were unconditionally a GAAP DCF/Graham/DDM blend for every sector including REITs, the exact distortion class this whole group exists to fix, just never actually fixed for this specific, highest-visibility output (fed directly into the screener, security-detail page, and — since RM4 — the AI thesis's own leading bear-case claim). The original definition doc (`specs/sector-aware-valuation-metrics.md` line 17) had already identified "MoS | Composite fair value" as a distorted metric in its own table; RM2's workaround (a P/FFO percentile-vs-own-history band) fixed only the Value Score's internal MoS *pillar*, never `ValuationResult` itself — flagged in RM2's own `plan.md` as a "concern the AI thesis / UI must not blur," but never re-surfaced once RM4 built the AI Thesis integration on top of the still-untouched field.
- Added a `P_AFFO` valuation band (mirrors RM2's `P_FFO` band exactly, additive, zero non-REIT impact) and a new `ValuationResult.affoFairValue` column (Flyway `V30`). For a REIT-classified security, `compositeFairValue`/`marginOfSafety` are now computed live from `medianHistoricalP/AFFO × currentAFFO/share` — replacing, not supplementing, the GAAP blend, so the screener, security-detail page, and AI thesis all inherit the fix from the one entity they already share, with **zero code change needed in `ThesisInputBuilder`**. `dcfFairValue`/`grahamNumber`/`ddmFairValue` remain computed and persisted for REIT securities unchanged. When AFFO history is insufficient, the REIT's headline valuation is `null` — never a silent fallback to the distorted GAAP composite.
- Live-verified against the real Vertex AI Gemini staging deployment: `O`'s `compositeFairValue`/`marginOfSafety` flipped from `49.98`/`-23.05%` (`OVERVALUED`) to `62.83`/`+2.33%` (`FAIR_VALUE`) after this phase, confirmed by direct `psql` inspection of the persisted `valuation_result` row; `AAPL`'s figures stayed byte-for-byte unchanged, confirming the non-REIT path is genuinely untouched. A regenerated AI thesis for `O` on staging (real Gemini call, post-redeploy) flipped its classification from `POTENTIALLY_OVERVALUED` to `POTENTIALLY_UNDERVALUED`, with the bull case now correctly citing a positive `marginOfSafetyPercent` — zero code change needed in `ThesisInputBuilder`, confirming the fix propagates end to end through the one shared `ValuationResult` entity. Full detail: `specs/2026-09-03-rm5-reit-composite-fair-value/validation.md`.
