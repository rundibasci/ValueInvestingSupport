...
### Google sign-in wired into K2 dev, live-tested, and torn down — 2026-08-27

Follow-up pass, separate from the merge gate above: Group J's Google OIDC backend (J1) was complete but never wired into K2's Terraform, so Google sign-in was silently disabled on Cloud Run. Wired `google-client-id`/`google-client-secret` into the `secret-manager` module and `GOOGLE_REDIRECT_URI`/`GOOGLE_FRONTEND_CALLBACK` into `cloud-run-service`, for `dev` only (staging untouched).

- Re-provisioned `dev` from a clean state (network-layer residue from the prior teardown was reused; everything else — Cloud SQL, Redis, secrets, IAM, Cloud Run service/jobs, Scheduler — created fresh).
- Created a Google Cloud OAuth client for the account's personal (non-Workspace) project: Console only offers **External** user type, not Internal — kept in **Testing** publishing status with the test operator's email as an explicit test user, which avoids Google's verification review while still restricting sign-in to named accounts. First attempt used a **Desktop app** client type by mistake, which has no configurable redirect URI at all (`redirect_uri_mismatch`); recreated as **Web application** with the exact Cloud Run redirect URI, which fixed it.
- **Assumed Cloud Run URL format was wrong.** `environments/dev/main.tf` originally predicted the callback URL as `vis-k2-dev-api-<projectNumber>.<region>.run.app` (Cloud Run v2's newer deterministic format), computed before the service existed to avoid a self-referencing apply. The real assigned URL used the older random-hash format (`vis-k2-dev-api-ughjapmueq-ew.a.run.app`) instead — both formats resolve to the same service once it exists, but only the real one was valid as a Google-registered redirect URI. Fixed by hardcoding the real observed URL (see the comment left in `main.tf` for what to recheck if the service is ever recreated).
- **Real bug found via a live login attempt, not by any existing test:** repeat Google sign-in (every login after the first) 500'd with `LazyInitializationException` — `OAuthAccountResolver.resolve()`'s reused-identity path returned an uninitialized lazy `User` proxy that `OAuthLoginSuccessHandler` then accessed outside the closed transactional session. Fixed with `Hibernate.initialize(user)`; full backend suite + targeted OAuth tests pass. Neither `OAuthAccountResolverTest` nor `OAuthLoginSuccessHandlerTest` (both mock- or H2-backed) caught this — it only surfaces with a real Hibernate lazy proxy across a real transaction boundary, which those tests don't reproduce.
- **UX bug found via the live product test:** "Universe Curation" (backend: `ROLE_ADMIN`-gated) was listed in `AppShell.tsx`'s general nav instead of the `adminNavigation` list already used for Jobs/Data Fallbacks/Users, so every signed-in role saw the link and got a bare Forbidden on click. Fixed by moving it into `adminNavigation`.
- **Secret-population pitfalls hit and fixed live:** `FMP_API_KEY` was first pushed to Secret Manager via `cut | gcloud secrets versions add --data-file=-`, which preserves the source line's trailing newline; the embedded `\n` inside the `apikey` HTTP header value made FMP reject every request (`Validation failed for header 'apikey'`). Re-pushed with `printf '%s' | gcloud secrets versions add ...` (no trailing newline) — the same pattern already used correctly for the Google client id/secret and JWT keys.
- Verified Google sign-in end-to-end after both fixes: account creation on first login (role `INVESTOR`, per J1's design — Google sign-in never grants `ADMIN`/`ADVISOR`), repeat login after logout (identity-reuse path), and a manually-promoted `ADMIN` user for the product demo.
- Local docker builds for this pass needed `--platform linux/amd64` explicitly — the operator's machine is Apple Silicon (arm64), and Cloud Run rejected an arm64-only image manifest outright.
- Public network-level access (`allUsers` → `roles/run.invoker`) was granted for the live browser test and revoked immediately after, same discipline as the prior K2 pass.
- **Torn down after the demo**: `terraform destroy` on `dev` removed all 66 resources — confirmed by a project-wide sweep (Cloud SQL, Redis, Cloud Run services/jobs, Scheduler jobs, secrets, service accounts all empty). Hit the same known Google-side `servicenetworking` peering propagation delay documented for K1/K2's prior teardowns; the non-billable network residue (`vis-k2-dev-network`, subnet, peering, IP reservation) is pending Google's release, same as before.
- Not yet cleaned up: two ad hoc `wip-*` images pushed to Artifact Registry during this pass (`vis-k2/vis-backend:wip-20260827120339`, `...-amd64`) — minor storage cost, not urgent.
- Staging was not touched in this pass — Google sign-in remains unwired there.
