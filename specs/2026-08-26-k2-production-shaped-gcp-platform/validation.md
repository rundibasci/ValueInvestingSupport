...
- **`allUsers` still holds `roles/run.invoker` on `vis-k2-dev-api`** from the hands-on test session above — must be revoked before teardown/handoff; it was never a K2 design decision, only a debugging expedient.
