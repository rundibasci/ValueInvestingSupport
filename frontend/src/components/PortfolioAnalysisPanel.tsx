...
export function PortfolioAnalysisPanel({ portfolioId, importId, compact = false }: { portfolioId: string; importId?: string; compact?: boolean }): JSX.Element {
  ...
  return <section className={compact ? "mt-3 rounded-lg border border-slate-700 bg-slate-950/50 p-3" : "rounded-2xl border border-emerald-400/20 bg-slate-900/50 p-5 sm:p-6"} aria-label="Portfolio in-depth analysis">
    ...
    {run && <div className="mt-4"><div className="flex flex-wrap justify-between gap-2 text-sm"><span className="font-semibold text-emerald-200">{run.status} · {run.phase.replaceAll("_", " ")}{run.currentSymbol ? ` · ${run.currentSymbol}` : ""}</span><span className="text-slate-400">{run.processed}/{run.total} processed · {run.succeeded} complete · {run.partial} partial · {run.failed} failed</span></div><div className="mt-2 h-2 overflow-hidden rounded bg-slate-800"><div className="h-full bg-emerald-400 transition-all" style={{ width: `${progress}%` }} /></div><p className="mt-2 text-xs text-slate-400">Analysis {run.analysisVersion} · updated {new Date(run.updatedAt).toLocaleString()}{run.analyticsSnapshotId ? " · portfolio measurements refreshed" : ""}</p>{run.terminalReason && <p className="mt-2 text-sm text-amber-200">{run.terminalReason}</p>}</div>}
    {outcomes.data?.content.length ? <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[42rem] text-left text-sm"><thead className="text-xs uppercase text-slate-400"><tr><th className="py-2">Security</th><th>Status</th><th>Source / freshness</th><th>Availability</th><th>Research</th></tr></thead><tbody className="divide-y divide-slate-800">{outcomes.data.content.map((item) => <tr key={item.symbol}><td className="py-2 font-semibold text-white">{item.symbol}</td><td className={item.status === "FAILED" ? "text-rose-200" : item.status === "PARTIAL" ? "text-amber-200" : "text-emerald-200"}>{item.status}</td><td className="text-slate-300">{item.source || "Unavailable"}<span className="block text-xs text-slate-400">{item.refreshedAt || "No refresh date"} · {item.calculationVersion}</span>{item.sourceLastPrice != null && <span className="block text-xs text-slate-400">Broker {item.sourceLastPrice.toLocaleString()} → platform {item.refreshedPrice?.toLocaleString() ?? "—"}{item.priceVariancePercent != null ? ` (${item.priceVariancePercent.toFixed(2)}%)` : ""}</span>}{item.sourceBaseValue != null && <span className="block text-xs text-slate-400">Broker base value {item.sourceBaseValue.toLocaleString()}</span>}</td><td className="max-w-xs text-xs text-slate-400">{item.errorMessage || item.reason || item.fallbackReason || "Available"}</td><td>{item.reviewPath ? <Link to={item.reviewPath} className="text-emerald-200 underline">Full review</Link> : <span className="text-slate-600">Unavailable</span>}</td></tr>)}</tbody></table></div> : null}
    ...
    <p className="mt-4 text-xs leading-5 text-slate-400">Fair values, margins of safety, scores, risk labels, and rebalancing diagnostics are decision support, not personalized investment advice. Review assumptions, source dates, coverage, and model guardrails.</p>
  </section>;
}
