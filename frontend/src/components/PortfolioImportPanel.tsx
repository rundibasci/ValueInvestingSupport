...
function MappingControl({ rowId, isin, productName, onMap }: { rowId: string; isin: string | null; productName: string; onMap: (rowId: string, result: SecuritySearchResult) => void }): JSX.Element {
  ...
  return <div className="mt-2 min-w-64">
    ...
    {!search.isFetching && search.data?.length === 0 && <p className="mt-2 text-xs text-slate-400">No match — refine the search above.</p>}
    <div className="mt-2 space-y-1">
      {search.data?.map((result) => <button key={result.id} type="button" onClick={() => onMap(rowId, result)} className="block w-full rounded border border-slate-700 p-2 text-left text-xs text-slate-200 hover:border-emerald-400">
        <strong className="text-emerald-200">{result.symbol}</strong> · {result.companyName}<span className="block text-slate-400">{[result.exchange, result.sector].filter(Boolean).join(" · ") || "Profile context unavailable"}</span>
      </button>)}
    </div>
  </div>;
}
...
export function PortfolioImportPanel({ portfolios, activeId, onCommitted }: { portfolios: Portfolio[]; activeId: string | null; onCommitted: (portfolioId: string) => void }): JSX.Element {
  ...
  return <section className="rounded-2xl border border-emerald-400/30 bg-slate-900/70 p-5 sm:p-6" aria-labelledby="portfolio-import-title">
    ...
    {preview && <div className="mt-5">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-lg bg-slate-950/60 p-3"><span className="text-xs text-slate-400">Rows</span><strong className="block text-white">{preview.sourceRowCount}</strong></div><div className="rounded-lg bg-slate-950/60 p-3"><span className="text-xs text-slate-400">Base total</span><strong className="block text-white">{formatNumber(preview.baseValueTotal, preview.baseCurrency)}</strong></div><div className="rounded-lg bg-slate-950/60 p-3"><span className="text-xs text-slate-400">Warnings / errors</span><strong className="block text-white">{preview.warningCount} / {preview.errorCount}</strong></div><div className="rounded-lg bg-slate-950/60 p-3"><span className="text-xs text-slate-400">Expires</span><strong className="block text-white">{new Date(preview.expiresAt).toLocaleString()}</strong></div></div>
