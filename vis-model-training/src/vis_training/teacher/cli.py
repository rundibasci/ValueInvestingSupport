...
def _parser():
    ...
    smoke = commands.add_parser("smoke-plan"); smoke.add_argument("--scenarios", type=Path, required=True); smoke.add_argument("--output", type=Path, required=True); smoke.add_argument("--count", type=int, default=20)
...
def main(argv=None):
    ...
    try:
        ...
        elif args.command == "smoke-plan": result = write_smoke_plan(args.scenarios, args.output, args.count)
